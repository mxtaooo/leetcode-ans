...
let solveSudoku (board: char array array): unit = 
    ...
    let rec solve () =
        let ggg = 
            ...
            |> Seq.ofArray 
            |> Seq.indexed 
            |> Seq.collect (fun (row, chars) -> chars |> Seq.ofArray |> Seq.indexed |> Seq.map (fun (column, char) -> ((row, column), char))
            |> Seq.filter (fun ((r, c), char) -> char = '.')
            |> 
        ...
        let ppp =
            ...
            |> Seq.tryHead
            |> Option.map fst
        let mutable (row, column) as position = (-1, -1)
        for i = 0 to 8 do
            for j = 0 to 8 do
                if board.[i].[j] = '.' then
                    position <- (i, j)
